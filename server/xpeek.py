...
the instrument name, though in the case of polarized NGD and CGD
...
class Data1D(object):
    ...
    def add_point(self, fields):
        ...
        for f in fields:
            try:
                ...
                if f.strip() != '':
                    logging.error('could not parse "%s" as key=value pair'%f)
                continue
            values = [asfloat(d) for d in v.replace(',',' ').split()]
            if len(values) == 1: values = values[0]
            if k in columns:
                if self.instrument == 'BT7' and k == "E":
                    continue  # Skip known error that won't be fixed
                logging.error('duplicate field %s in %s (ignored)\n %s'
                              %(k, self.instrument, '\t'.join(fields)))
            else:
                columns[k] = values

        # BT7 hacks
        # - convert the 48 PSD fields into data list
        if self.instrument == 'BT7':
            if 'AnaDetectorSum' in columns:
                columns['DATA'] = columns['AnaDetectorSum'] 
            #elif 'PSDC00' in columns:
            #    columns['DATA'] = [columns['PSDC%02d'%i] for i in range(48)]
            #    for i in range(48): del columns['PSDC%02d'%i]
            #elif 'Det' in columns:
            #    columns['DATA'] = columns['Det']
        ...
        if self.instrument in ['NG2','NG4','BT8'] and 'PSD' in columns:
            columns['DATA'] = columns['PSD']


        # Make sure there is a data column
        if 'DATA' not in columns: columns['DATA'] = columns.get('roi',0)

        # How big is the detector?
        pixels = 1 if numpy.isscalar(columns['DATA']) else len(columns['DATA'])

        # Update array
        if len(self.columns) == 0 or int(columns['PT']) == 1:
            # Adding point to new dataset
            self.columns = dict((k,[v]) for k,v in columns.items())
            self.pixels = pixels
        else:
    ...
    def eta(self):
        """
        Returns the estimated time to completion as a formatted string %H:%M[+#days].
...
def fit_exp(x,y,dy=None,drop=2):
    ...
    if y[n] > y[0] and y[k] > y[0] and y[n] != y[k]:
...
class XPeek(object):
    ...
    def parseline(self,line):
        ...
        if fields[0].endswith('END'):
            ...
            # To support multiline interleaved scans, keep track of whether
            # the last set of scans has finished, and clear them out when
            # the next set of scans starts. 
            if self.between_scans: self.data = {}
            self.between_scans = False
...
if __name__ == "__main__":
    ...
    instrument = sys.argv[1] if len(sys.argv)>1 else 'NGD'
    demo(instrument)
