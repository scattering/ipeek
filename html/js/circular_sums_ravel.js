            ...
            if (dx > 0 && dy > 0) {                
                ...
                for (var i=0; i<dx; i++) {
                    for (var j=0; j<dy; j++) {
                        ...
                        //sum += n * data[(ch - min_y - j -1) * cw + (min_x + i)];
                        // switch to C-order
                        sum += n * data[(ch - min_y - j - 1) + ch*(min_x + i)];
                    }
                }
            }
            angle_list[s] = angle;
            sum_list[s] = sum/255;
            normalize_list[s] = normalize/255;
    ...
    function multiplySumRadial(maskCanvas, data, dims, center_x_coord, center_y_coord) {
         ...
         for (var s=0; s<steps; s++) {            
            ...
            radius_list[s] = r;
            sum_list[s] = sum/255; // divide by 255 to get raw counts.
            normalize_list[s] = normalize/255; // 0 to 255;
