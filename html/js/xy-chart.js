  ...
  function xyChart(options_override) {
    ...
    function chart(selection) {
      selection.each(function (data) {
        ...
        xAxisGrid.scale(x).tickSize(-height).ticks(options.numberOfTicks).tickPadding(10).tickFormat("");

        yAxisGrid.scale(y).tickPadding(10).ticks(options.numberOfTicks).tickSize(-width).tickFormat("");
        ...
        mainview.append("rect").classed("zoom-box", true).attr("width", width).attr("height", height).style("visibility", "hidden").attr("pointer-events", "all");
        ...
        if (options.position_cursor) {
          var pcurse_selection = mainview.selectAll(".position-cursor").data([0]);
          var position_cursor = pcurse_selection.enter().append("text").attr("class", "position-cursor").attr("x", width - 10).attr("y", height - 10).style("text-anchor", "end");
    ...
    chart.resetzoom = function () {
      ...
      zoombox.call(zoom.transform, d3.zoomIdentity.translate(xoffset, yoffset).scale(1.0 - 2 * base_zoom_offset));
      is_zoomed = false;
    };
    ...
    chart.source_data = function (_) {
      ...
      source_data = _;
      do_autoscale();
      if (!is_zoomed && options.autoscale) {
        chart.resetzoom();
      }
      return chart;
    };

    chart.is_zoomed = function () {
      return is_zoomed;
    };
